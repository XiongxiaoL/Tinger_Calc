...
public class P_F2 extends Fragment implements View.OnClickListener{
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view=inflater.inflate(R.layout.p_f2,null);
        initView(view);
        setOnClickListener();
    ...
    private void count() {
        ...
        if(numData == 2) {
            ...
            switch (res) {
                case "x":
                    divisor = dw * dr;
                    if (divisor != 0) x = decimalFormat.format((2 * dw - dr) / divisor);
                    else {
                        Toast.makeText(getActivity(), "数据格式有误", Toast.LENGTH_SHORT).show();
                        x = null;
                    }
                ...
                case "w":
                    divisor = 2 * dx * dr;
                    if (divisor != 0) w = decimalFormat.format((2 * dx - dr) / divisor);
                    else {
                        Toast.makeText(getActivity(), "数据格式有误", Toast.LENGTH_SHORT).show();
                        w = null;
                    }
                    break;
                case "r":
                    r = decimalFormat.format(2 * dx * dw / (dx + dw));
                    break;
